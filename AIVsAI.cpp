...
coordinates alpha_beta_search(int maximizing_player)
{
	return_struct v;
	if (maximizing_player == PLAYER_ONE)
		v = min_value(-1 * INF, INF);
	else
		v = max_value(-1 * INF, INF);
    return v.action;
}
...
int main()
{
    ...
    cout << "Program that plays 4 x 4 Tic Tac Toe" << endl << endl;
    ...
    for (int i = 0; i < 8; i++)
    {
		...
		if (squares_remaining == 16)
		{
			x_coord = 0;
			y_coord = 1;	// optimal first move
		}
		else
		{			
			coordinates player1_move = alpha_beta_search(PLAYER_ONE);
			x_coord = player1_move.x;
			y_coord = player1_move.y;				
		}
		cout << "\nPlayer 1 marks (" << x_coord << ", " << y_coord << ")" << endl;

        board[x_coord][y_coord] = 0;
        squares_remaining--;

        coordinates adv_move = alpha_beta_search(PLAYER_TWO);	// adversary's move
        x_coord = adv_move.x;
        y_coord = adv_move.y;
        board[x_coord][y_coord] = 1;
        squares_remaining--;
        
		cout << "Player 2 marks (" << x_coord << ", " << y_coord << ")" << endl;
